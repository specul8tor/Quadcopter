...
#define PWR_CONF_ADV_PWR_SAVE 0x7c
...
#ifdef CONFIG_FILE
  uint16_t *init_array = &bmi270_config_file[0];
  while(!i2c_write_to_register(bmiI2CAddr, PWR_CONF_ADV_PWR_SAVE, 0x00, &bmi_pins));
  sys_clock_wait(1382);//450us
  while(!i2c_write_to_register(bmiI2CAddr, INIT_CTRL, 0x00, &bmi_pins));
  while(!i2c_write_buf_to_register(bmiI2CAddr, INIT_DATA, init_array,config_size,&bmi_pins));
  while(!i2c_write_to_register(bmiI2CAddr, INIT_CTRL, 0x01, &bmi_pins));
#endif

  //check that INT_STATUS REGISTER == 0 pg21
  wait_ms(50);
  uint16_t status = i2c_read_value(bmiI2CAddr, INTERNAL_STATUS, 1, &bmi_pins);
  print_int("Status Check: ", status, XPD_Flag_Hex, 1);

  //config to performance mode pg22
  i2c_write_to_register(bmiI2CAddr, PWR_CTRL, 0x0E, &bmi_pins);
