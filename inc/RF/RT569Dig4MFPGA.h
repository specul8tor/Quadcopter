//
//  RT569DigFPGA4M.h
//  4Mbps Driver for FPGA emulation 
...
#ifndef LIBSPINE_COMMON_RT569DIG4M_RADIO_H_
#define LIBSPINE_COMMON_RT569DIG4M_RADIO_H_
...
struct StandardRT569Dig4MConfigParams {
...
struct RT569Dig4MConfigWithFullInitParams {
  static constexpr size_t INITIAL_PREAMBLE = 5;
  static constexpr uint16_t PLL_TO_TX_TIME_IN_BBU_TICKS = 216;
  static constexpr uint16_t RSSI_WAIT_TIME_IN_BBU_TICKS = 100;
...
struct RT569Dig4MConfigWithModemInitParams {
  static constexpr size_t INITIAL_PREAMBLE = 5;
  static constexpr uint16_t PLL_TO_TX_TIME_IN_BBU_TICKS = 216;
  static constexpr uint16_t RSSI_WAIT_TIME_IN_BBU_TICKS = 100;
...
static constexpr std::array<std::array<uint16_t, 3>, 3> init_a_la_cart = {{
    ...
    {{0x43C >> 2, 0x0000, 0xE0FE}},
    {{0x450 >> 2, 0x0000, 0x0003}},
}};

static constexpr std::array<std::array<uint16_t, 3>, 10> settings_4096_mode = {{
    ...
    {{0x43C >> 2, 0x0000, 0xE0FE}}, // adda_slk_sel
}};
} // namespace RT569Dig4MDetail

template <class Config, class PAControlTy, class SPIwCS, class BBUTy,
          class RadioPort, class BBUPinTy>
class RT569DigFPGA4M
    : public IRadio<
          RT569DigFPGA4M<Config, PAControlTy, SPIwCS, BBUTy, RadioPort, BBUPinTy>> {
public:
  using BBU = IBaseBandUnit<BBUTy>;
  using BBUPin = IOutputPin<BBUPinTy>;

  RT569DigFPGA4M(SPIwCS &spi, BBU &bbu) : spi_(spi), bbu_(bbu) {
  ...
  void Set32MClock() { 
    WriteReg(0x43C >> 2, 0x0000, 0xE0FE); 
    xout << "Radio clock output now 32.768 MHz\r\n";
    }
  ...
  void ChangeChannel(size_t channel) {
    ...
    WriteReg(0x300 >> 2, (RT569Dig4MDetail::channel_settings[channel][0] << 6) | 15,
             RT569Dig4MDetail::channel_settings[channel][1]);
  }
  ...
  void SwitchToTransmit(size_t power_level) {
    ...
    //WriteReg(0x3A8 >> 2, 0x0000, RT569Dig4MDetail::power_settings[power_level]);
    //switch to tx tho
  ...
  uint16_t GetRSSI() {
    ...
    auto rssi = rssi_post_agc - RT569Dig4MDetail::lna_gain_codes[lna] -
                RT569Dig4MDetail::tia_gain_codes[tia] -
                RT569Dig4MDetail::vga_gain_codes[vga];
...
private:
  ...
  void ConfigureRadio() {
    ...
    if (Config::INIT_RF) {
      WriteInitData(RT569Dig4MDetail::kRfDataBaseAddress, RT569Dig4MDetail::kRfInitData);
    }

    if (Config::INIT_MODEM) {
      //this is the write order in their verilog tb
        for (size_t i = 0; i < RT569Dig4MDetail::init_a_la_cart.size(); ++i) {
          WriteReg(RT569Dig4MDetail::init_a_la_cart[i][0],
                   RT569Dig4MDetail::init_a_la_cart[i][1],
                   RT569Dig4MDetail::init_a_la_cart[i][2]);
      }
      WriteInitData(RT569Dig4MDetail::kModemDataBaseAddress,
                    RT569Dig4MDetail::kModemInitData);
    }

    if (Config::INIT_PMU) {
      WriteInitData(RT569Dig4MDetail::kPmuDataBaseAddress,
                    RT569Dig4MDetail::kPmuInitData);
    }

    for (size_t i = 0; i < RT569Dig4MDetail::settings_4096_mode.size(); ++i) {
      WriteReg(RT569Dig4MDetail::settings_4096_mode[i][0],
               RT569Dig4MDetail::settings_4096_mode[i][1],
               RT569Dig4MDetail::settings_4096_mode[i][2]);
    }
  }
...
constexpr RT569DigFPGA4M<Cfg, Pa, S, B, Port, Pin>
MakeRT569DigFPGA4M(Cfg, Pa &pa, S &spi, B &bbu, Port &, Pin &) {
  return {spi, bbu};
}
